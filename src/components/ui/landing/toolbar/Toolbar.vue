<template>
  <div>
    <v-navigation-drawer v-model="drawer" app temporary>
      <v-list dense nav>
        <v-subheader class="text-uppercase font-weight-bold">Battleline</v-subheader>
        <v-list-item v-for="(item, index) in menu" :key="index" :disabled="item.disabled" :to="item.link">
          <v-list-item-content>
      ...
      <template v-slot:append>
        <div class="pa-2">
          <v-btn disabled block class="mb-2">
            Sign In
          </v-btn>
    ...
    <v-app-bar app height="80">
      <v-container class="py-0 px-0 px-sm-2 fill-height" :fluid="isFluid">
        ...
        <v-btn large disabled text class="mx-1 d-none d-sm-inline-block">
          Sign In
        </v-btn>
        <v-btn color="primary" disabled large>
          Sign Up
        </v-btn>
      </v-container>
    </v-app-bar>
